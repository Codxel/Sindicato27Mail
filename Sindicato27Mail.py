...
while True:
    ...
    for i in range(num_mensajes + 1, num_mensajes + nuevo_num_mensajes + 1):
        ...
        id_personal = consulta.ejecutar_consulta("SELECT id_personal FROM users WHERE email = %s", [
                                                 re.search(r'<(.*?)>', mensaje['From']).group(1)])
        if id_personal:
            id_tipopersonal = consulta.ejecutar_consulta(
                "SELECT id_tipopersonal FROM personal WHERE id = %s", [id_personal[0][0]])
            asunto = mensaje['Subject']
            asunto = asunto.replace('\r', '').replace('\n', '')
            # Procesar el mensaje utilizando la función procesar_comando
            print("NUEVO MENSAJE: " + asunto)
            tablas.procesar_comando(
                asunto, mensaje['From'], id_tipopersonal[0][0]-1)

        else:
            correo.enviar_correo(
                'REMITENTE NO REGISTRADO', 'El remitente no está registrado en la base de datos, comuniquese con el administrador.', mensaje['From'])
